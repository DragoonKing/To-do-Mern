...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
    ...
    useEffect(() => {
        if (edit && isAddTaskModalOpen) {
            axios.get(`https://to-do-backend-rli3.onrender.com/project/${projectId}/task/${taskId}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`https://to-do-backend-rli3.onrender.com/project/${projectId}/task`, { title, description: desc })
                .then((res) => {
            ...
            axios.put(`https://to-do-backend-rli3.onrender.com/project/${projectId}/task/${taskId}`, { title, description: desc })
                .then((res) => {
                    setAddTaskModal(false)
