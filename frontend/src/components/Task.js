...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`https://to-do-backend-rli3.onrender.com/project/${projectId}`)
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`https://to-do-backend-rli3.onrender.com/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`https://to-do-backend-rli3.onrender.com/project/${projectId}/task/${taskId}`)
            .then((res) => {
